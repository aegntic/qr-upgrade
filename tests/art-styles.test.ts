...
test('every selectable material passes both API boundaries and keeps the front-facing generation constraint', async () => {
  for (const style of Object.keys(ART_STYLES) as ArtStyle[]) {
    ...
    const request = new Request('https://qrupgrade.com/api/art', { method: 'POST', headers: { origin: 'https://qrupgrade.com', 'content-type': 'application/json', cookie: 'qr-art-session=' + 'c'.repeat(64), 'x-vercel-forwarded-for': '192.0.2.1' }, body: JSON.stringify({ requestId: input.id, prompt: input.prompt, style }) });
