...
test('non-canonical host redirects to apex unless ACCOUNT_ORIGIN allowlists it',async()=>{
 const apex={...config,development:false};
 const bounce=await login(new Request('https://qr-upgrade-web.aegntic.workers.dev/api/account/login'),apex);
 assert.equal(bounce.status,303);
 assert.equal(bounce.headers.get('location'),'https://qrupgrade.com/api/account/login');
 const cf={...apex,accountOrigin:'https://qr-upgrade-web.aegntic.workers.dev'};
 const stay=await login(new Request('https://qr-upgrade-web.aegntic.workers.dev/api/account/login'),cf);
 const url=new URL(stay.headers.get('location')!);
 assert.equal(url.origin,'https://accounts.google.com');
 assert.equal(url.searchParams.get('redirect_uri'),'https://qr-upgrade-web.aegntic.workers.dev/api/account/callback');
});
