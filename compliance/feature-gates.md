...
The web beta implements local design workflows, bounded live AI generation, credential-gated individual cloud accounts, dynamic destinations and hosted-content builders. Billing requires merchant configuration and explicit activation. These are not enterprise assurance claims. G19 blocks representing the enterprise capabilities below as complete until implementation and evidence exist. See runtime-data-flow.md for current technical scope.
...
| Hosted pages, files and forms | Explicit publication, safe structured renderer, private object membership checks, upload validation, attachment isolation, spam quotas, approved notices/retention, data rights handling and abuse reporting. Technical controls alone do not approve collected content or legal basis. |
