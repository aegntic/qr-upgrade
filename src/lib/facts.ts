...
export const faqs = [
  ...
  },
  {
    ...
    a: "QR rendering happens locally. Save design stores a browser copy; When account sign-in is available, Save to cloud explicitly sends the complete design and images to your private account storage. Hosted page uploads are also stored privately until you publish them. Cloud storage and AI generation use Cloudflare; generation sends only the prompt and style. Generated results are available for one hour and removed by hourly cleanup; hashed job and quota metadata is retained for up to three days. Clearing site data removes local copies only. Anyone scanning a Wi-Fi QR can read its credentials.",
  },
  {
  ...
  {
    q: "Can I change a destination after sharing its QR?",
    a: "With a signed-in account, create and publish a Dynamic link first, then design its QR. Your Account page shows whether sign-in is available. You can update the target without changing that QR, pause the link and see daily aggregate opens. Counts include repeats and bots. Ordinary static QR exports keep their encoded destination. Team workspaces and geographic routing are planned. Billing shows the current availability of paid plans.",
  },
];
