...
const billingDate=(seconds:number)=>new Date(seconds*1000).toLocaleDateString(undefined,{year:'numeric',month:'long',day:'numeric'});
...
export default function BillingPanel(){
 ...
 {status?.configured&&<>{!status.signedIn&&<div className="service-gate"><ShieldCheck size={24}/><div><strong>Your plan belongs to your account.</strong><p>Sign in before choosing or managing a subscription.</p></div><Link className="nav-cta" href="/account">Sign in <ArrowUpRight size={15}/></Link></div>}{status.entitlement&&!status.subscription&&<p className="service-notice">Current Free capacity: {capacity(status.entitlement.limits)}.</p>}{status.subscription&&<div className="billing-current"><div><span className="billing-label">YOUR SUBSCRIPTION</span><h2>{status.subscription.tier==='brand'?'Brand':'Pro'}</h2><p>{status.subscription.status.replaceAll('_',' ')}</p>{status.subscription.scheduledCancellationAt&&<small>Subscription scheduled to end {billingDate(status.subscription.scheduledCancellationAt)}</small>}<small>Current period ends {billingDate(status.subscription.currentPeriodEnd)}</small>{status.entitlement&&<small>Current capacity: {capacity(status.entitlement.limits)}</small>}</div>{status.canManage&&<button className="nav-cta" disabled={busy} onClick={()=>void open('portal')}>Manage subscription <ArrowUpRight size={16}/></button>}</div>}
