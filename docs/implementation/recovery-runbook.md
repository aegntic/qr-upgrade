...
## Deletion-aware historical restore rehearsal, 21 September 2026

The retained 20 September SQL/object snapshot was restored twice into disposable local workerd D1/R2 bindings using service source at `e6c2223`, with migrations through `0007` applied. Both original object hashes matched. In the first copy, a synthetic account closure used the actual service challenge/intent/acceptance routes and wrote the minimal independent receipt to local R2. No production account was closed.

Before exposing any endpoint from the second historical copy, the rehearsal copied that newer receipt independently and reconciled its closed-owner and session-version state into the restored database. All 29 checks passed: historical private owner isolation; immediate private/public denial after acceptance; restored closed identity refusing sign-in; restored designs/pages/redirects denied before content cleanup; bounded cleanup completion; removal of the old owned objects; preservation of the independent receipt; and a nondecreasing session version. Report: private `compliance/evidence/recovery/2026-09-21-lifecycle/runtime-verification.json`.

This uses actual local workerd D1/R2 storage and the bundled current service. It is not a remote production restore, a new production backup, an automated reconciliation endpoint, an approved RTO/RPO, or proof that a receipt lost before acknowledgement can be reconstructed. The script's first run had an incorrect fixture column name, corrected to the actual schema before the passing run; no product source was changed by the rehearsal.
