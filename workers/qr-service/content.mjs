...
export async function contentRequest(r,env){
 ...
 const limits=limitsForPlan(r.headers.get('x-qr-plan')??'free');if(!limits)return fail('Invalid plan.',400);
 ...
 try{
  if(r.method==='GET'&&!id){const rows=await env.DB.prepare('SELECT * FROM content_pages WHERE owner=? ORDER BY updated_at DESC').bind(owner).all();return reply({pages:rows.results.map(page),limit:limits.hostedPages});}
  if(r.method==='POST'){
...
export async function assetRequest(r,env){
 ...
 try{if(r.method==='GET'){if(id){const row=await env.DB.prepare('SELECT * FROM content_assets WHERE id=? AND owner=? AND ready=1').bind(id,owner).first();return row?await deliverAsset(env,row):fail('File not found.',404);}const rows=await env.DB.prepare('SELECT * FROM content_assets WHERE owner=? AND ready=1 ORDER BY created_at DESC').bind(owner).all();return reply({assets:rows.results.map(assetMetadata),limit:limits.uploadedAssets});}
