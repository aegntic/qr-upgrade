...
export async function contentRequest(r,env){
 ...
 try{
  if(r.method==='GET'&&!id){const rows=await env.DB.prepare('SELECT * FROM content_pages WHERE owner=? ORDER BY updated_at DESC').bind(owner).all();return reply({pages:rows.results.map(page),limit:50});}
  if(r.method==='POST'){
   ...
   const row=await env.DB.prepare("INSERT INTO content_pages(id,owner,slug,draft,created_at,updated_at) SELECT ?,?,?,?,?,? WHERE (SELECT COUNT(*) FROM content_pages WHERE owner=?)<50 RETURNING *").bind(crypto.randomUUID(),owner,slug,JSON.stringify(input),now,now,owner).first();return row?reply({page:page(row)},201):fail('Your account holds up to 50 pages, including archived pages.',409);
  }
...
export async function assetRequest(r,env){
 ...
 try{if(r.method==='GET'){if(id){const row=await env.DB.prepare('SELECT * FROM content_assets WHERE id=? AND owner=? AND ready=1').bind(id,owner).first();return row?await deliverAsset(env,row):fail('File not found.',404);}const rows=await env.DB.prepare('SELECT * FROM content_assets WHERE owner=? AND ready=1 ORDER BY created_at DESC').bind(owner).all();return reply({assets:rows.results.map(assetMetadata),limit:100});}
 const assetId=crypto.randomUUID(),key=`content/${crypto.randomUUID()}`,now=new Date().toISOString();const row=await env.DB.prepare('INSERT INTO content_assets(id,owner,name,mime,size,r2key,created_at) SELECT ?,?,?,?,?,?,? WHERE (SELECT COUNT(*) FROM content_assets WHERE owner=?)<100 RETURNING *').bind(assetId,owner,input.name,input.mime,bytes.length,key,now,owner).first();if(!row)return fail('Your account holds up to 100 files.',409);
