...
export async function cloudRequest(request,env){
 ...
 try{
  if(!id&&request.method==='GET'){const result=await env.DB.prepare('SELECT id,name,archived,created_at,updated_at FROM cloud_designs WHERE owner=? AND ready=1 ORDER BY updated_at DESC').bind(owner).all();return reply({designs:result.results.map(metadata)});}
  ...
  if(!id){const reserved=await env.DB.prepare('INSERT INTO cloud_designs(id,owner,name,created_at,updated_at,r2key) SELECT ?,?,?,?,?,? WHERE (SELECT COUNT(*) FROM cloud_designs WHERE owner=?) < 50 RETURNING id').bind(designId,owner,snapshot.name,now,now,r2key,owner).first();if(!reserved)return reply({error:'Your cloud library holds up to 50 designs, including archived designs.'},409);}
  try{
