...
export async function accountDeletionRequest(request,env){
 ...
 if(!Number.isSafeInteger(b.version)||b.version<1||b.version>=Number.MAX_SAFE_INTEGER)return reply({error:'Invalid request.'},400);
 if(path==='/account/deletion/challenge'){
  if(Object.keys(b).sort().join(',')!=='challenge,version'||!hash.test(b.challenge||''))return reply({error:'Invalid request.'},400);
  const row=await env.DB.prepare("INSERT INTO account_deletion_challenges(owner,challenge,version,expires_at) SELECT owner,?,session_version,? FROM account_security WHERE owner=? AND session_version=? AND lifecycle='active' ON CONFLICT(owner) DO UPDATE SET challenge=excluded.challenge,version=excluded.version,expires_at=excluded.expires_at,consumed=0 RETURNING challenge").bind(b.challenge,Date.now()+5*60000,owner,b.version).first();
  return row?reply({challenge:row.challenge}):reply({error:'Sign in again before confirming this account.'},409);
 }
 if(!hash.test(b.intent||''))return reply({error:'Invalid request.'},400);
 if(path==='/account/deletion/intent'){
  if(Object.keys(b).sort().join(',')!=='challenge,intent,version'||!hash.test(b.challenge||''))return reply({error:'Invalid request.'},400);
  const now=Date.now(),result=await env.DB.batch([
   env.DB.prepare('DELETE FROM account_deletion_intents WHERE owner=? AND consumed=0 AND EXISTS(SELECT 1 FROM account_deletion_challenges WHERE owner=? AND challenge=? AND version=? AND consumed=0 AND expires_at>=?)').bind(owner,owner,b.challenge,b.version,now),
   env.DB.prepare("UPDATE account_deletion_challenges SET consumed=1 WHERE owner=? AND challenge=? AND version=? AND consumed=0 AND expires_at>=? AND EXISTS(SELECT 1 FROM account_security WHERE owner=? AND session_version=? AND lifecycle='active') RETURNING challenge").bind(owner,b.challenge,b.version,now,owner,b.version),
   env.DB.prepare('INSERT INTO account_deletion_intents(id,owner,version,expires_at) SELECT ?,owner,version,? FROM account_deletion_challenges WHERE owner=? AND challenge=? AND changes()=1 RETURNING id').bind(b.intent,now+5*60000,owner,b.challenge)
  ]);
  return result[2].results.length?reply({intent:b.intent}):reply({error:'Confirm with Google again before closing this account.'},409);
 }
...
export async function cleanupAccount(env,row){
 ...
 // Independent cursors keep both journal enqueue and deletion attempts fair, even when protected keys remain.
 let uploads=await env.DB.prepare("SELECT r2key FROM account_uploads WHERE owner=? AND state='settled' AND r2key>? ORDER BY r2key LIMIT 25").bind(owner,row.uploads_cursor??'').all();
 if(!uploads.results.length&&row.uploads_cursor)uploads=await env.DB.prepare("SELECT r2key FROM account_uploads WHERE owner=? AND state='settled' ORDER BY r2key LIMIT 25").bind(owner).all();
 const stmts=[];
 ...
 for(const {r2key} of uploads.results)stmts.push(env.DB.prepare('INSERT OR IGNORE INTO account_deletion_objects(owner,r2key) VALUES(?,?)').bind(owner,r2key));
 if(uploads.results.length)stmts.push(env.DB.prepare('UPDATE account_deletions SET uploads_cursor=? WHERE id=?').bind(uploads.results.at(-1).r2key,row.id));
 ...
 let manifest=await env.DB.prepare('SELECT r2key FROM account_deletion_objects WHERE owner=? AND r2key>? ORDER BY r2key LIMIT 25').bind(owner,row.manifest_cursor??'').all();
 if(!manifest.results.length&&row.manifest_cursor)manifest=await env.DB.prepare('SELECT r2key FROM account_deletion_objects WHERE owner=? ORDER BY r2key LIMIT 25').bind(owner).all();
 for(const {r2key} of manifest.results){
  // Advance before attempting I/O: a failing or protected key must not monopolize every future tick.
  await env.DB.prepare('UPDATE account_deletions SET manifest_cursor=? WHERE id=?').bind(r2key,row.id).run();
...
export async function cleanupAccounts(env){
 ...
 await env.DB.prepare('DELETE FROM account_deletion_challenges WHERE owner IN (SELECT owner FROM account_deletion_challenges WHERE expires_at<? LIMIT 100)').bind(Date.now()).run();
